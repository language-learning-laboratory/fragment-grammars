...
(define (get-permutations gensyms labels)
  (if (and (null? gensyms) (null? labels))
      '(())
      (let ([remainder (get-permutations  (rest gensyms) (rest labels))])
        (let ([gs (first gensyms)]
              [lb (first labels)])
          (if (terminal? lb)
              (map (lambda (p) (pair gs p)) remainder) ;;terminals don't have gensyms, just extend everything by the terminal
              (append
               (map (lambda (p) (pair gs p)) remainder)
               (map (lambda (p) (pair lb p)) remainder)))))))
        
(define category-subtree-counts (make-hash-table))
(define DOP1-rule-counts (make-hash-table))
(define GDMN-rule-counts (make-hash-table))
...
(define (count-tree tree type-count root)
    (if (not (list? tree)) ; It is a terminal
        (let ([rhs-total-subtrees 0])
          (begin
            (hash-table-set! terminals tree 0)
            (hash-table-update! category-subtree-counts tree (lambda (old-count) (+ old-count rhs-total-subtrees)) (lambda () rhs-total-subtrees)) ; Thang 0))
            tree))
        (let* ([current-node-gensym (first tree)] ;;nonterminal
               [label (my-gensym->symbol current-node-gensym)]
               ;[dummy (begin (display "label ") (disp label))]
			   [children (rest tree)]
               [children-gensyms  (map (lambda (t) (count-tree t type-count #f)) children)]              
               [permutations (get-permutations children-gensyms (trees->labels children))]
               ...
               [new-rules (if (equal? label 'START)
                              (map (lambda (l) (pair label l)) permutations)
                              (append
                               (map (lambda (l) (pair current-node-gensym l)) permutations)
                               (map (lambda (l) (pair label l)) permutations)))]
							   )
          (begin (for-each (lambda (r)
                           (let* ([head (first r)]
                                  [rhs (rest r)]
                                  [rhs-counts (map (lambda (sym)
                                                     (if (inner? sym)
                                                         (hash-table-ref category-subtree-counts sym)
                                                         1)) rhs)]
                                  [rhs-total-subtrees (apply * rhs-counts)]
                                  [total-node-count (* rhs-total-subtrees type-count)])
                             (begin
                               (when (inner? head)
                                 (hash-table-update! category-subtree-counts
                                                     head
                                                     (lambda (old-count) (+ old-count rhs-total-subtrees))
                                                     (lambda () rhs-total-subtrees))) ; Thang 0)))
                               (hash-table-update! DOP1-rule-counts
                                                   (cons head rhs)
                                                   (lambda (old-count) (+ old-count  total-node-count)) (lambda () total-node-count)) ; Thang 0))
                               (let ([parent-num-subtrees (apply * (map  (lambda (x) (+ x 1))
                                                                         (map (lambda (sym)
                                                                                (if (inner? sym)
                                                                                    (hash-table-ref category-subtree-counts sym)
                                                                                    0)) rhs)))])
                                 (hash-table-update! GDMN-rule-counts
                                                     (cons head rhs)
                                                     (lambda (old-count) (+ old-count  (* (/ rhs-total-subtrees parent-num-subtrees) type-count))) (lambda () (* (/ rhs-total-subtrees parent-num-subtrees) type-count))))))) ; Thang 0))))))
                         new-rules)
               current-node-gensym)))) 
...
;; (define (remove-unity-prob-rules grammar)
;;   (let ([unity-prob-rules (make-hash-table)]
;;         [new-grammar (make-hash-table)])
;;     (begin
;;       (hash-table-walk
;;        grammar
;;        (lambda (head rhss)
;;          (when (not (eq? head 'START))
;;            (let ([total 0])
;;              (hash-table-walk
;;               rhss
;;               (lambda (rhs prob)
;;                 (when (= prob 1.0) (hash-table-set! unity-prob-rules head rhs)))))))) ;; must not clobber the start symbol!
      
;;       (hash-table-walk
;;        grammar
;;        (lambda (head rhss)
;;          (when (not (hash-table-exists? unity-prob-rules head)) ;;delete the rules that were at unity
;;            (let ([new-rhss (make-hash-table)])
;;              (begin
;;                (hash-table-walk
;;                 rhss
;;                 (lambda (rhs prob)
...
;;                                   (map
;;                                    (lambda (symbol)
;;                                      (if (hash-table-exists? unity-prob-rules symbol)
;;                                          (hash-table-ref unity-prob-rules symbol)
;;                                          symbol))
;;                                    rhs))])
;;                     (hash-table-update! new-rhss new-rhs (lambda (old-prob) (+ old-prob prob)) (lambda () 0)))))
;;                (hash-table-set! new-grammar head new-rhss))))))
;;       new-grammar)))

(define (remove-unity-prob-rules grammar) grammar)

;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; Create different grammar types ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (make-GDMN-grammar rules)
  (let ([total-per-category (make-hash-table)]
        [result-grammar (make-hash-table)])
    (begin
        (disp "Making GDMN grammar")
        ...
        (hash-table-walk rules
                         (lambda (rule rule-count )
                           (let* ([head (first rule)]
                                  ...
                                  [prob (/ rule-count category-total)])
...
(define (make-DOP1-grammar rules)
  (let ([total-per-category (make-hash-table)]
        [result-grammar (make-hash-table)])
    (begin
        (disp "Making DOP1 grammar")
        (hash-table-walk rules
                         (lambda (rule rule-count )
                           (let ([head (first rule)]
                                 [rhs (rest rule)])
...
(let* ([prefix (second (command-line))]
  ...
  (begin
                                   ...
                                   (set! treeCount (+ treeCount 1))
                                     ...
                                     (begin
                                       (display " (")(display treeCount)(display ") ")))
			                             ;;(display "# ")(display treeCount)(display " ")(disp tree)	
    ...
    (let* ([mdpcfg (make-mdpcfg pi minimal-rule-counts)]
      ...
      (print-grammar (make-DOP1-grammar DOP1-rule-counts) (open-output-file  (string-append outputdir  "/" out-prefix  ".DOP1-gram.txt")))
      (print-grammar (make-GDMN-grammar GDMN-rule-counts) (open-output-file  (string-append outputdir  "/" out-prefix  ".GDMN-gram.txt")))
	  )))
